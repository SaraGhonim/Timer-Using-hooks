...
class ExistingPatient extends Component {
  existingPatient() {
    ...
    return (
      <LinearGradient
        ...
        style={[blockStyles.Text, cardStyles.bold, styles.awards]}
        colors={['#FF955A', theme.colors.accent]}>
          
        <Block center>
          <Text  size={theme.sizes.title} spacing={2} center bold  white>
            Existing Patient
          </Text>
        </Block>
      </LinearGradient>
  ...
  render() {
    return <React.Fragment>{this.existingPatient()}</React.Fragment>;
  }
}
